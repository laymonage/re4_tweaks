...
void re4t::init::Gameplay()
{
	...
	if (re4t::cfg->bBalancedChicagoTypewriter)
	{
		...
		auto pattern = re4t::pattern("8B 5D ? 39 5D ? 72 ? E8 ? ? ? ? 0F B6 ? 81 E2 ? ? ? ? 79 ? 4A 83 CA ? 42 83 FA ? 0F 85");
		...
		pattern = re4t::pattern("D9 04 8D ? ? ? ? D9 5D ? 75 ? 8B CE 83 E9");
		...
		*/
		...
		pattern = re4t::pattern("BE ? ? ? ? F7 42 ? ? ? ? ? 74 ? 83 C0 ? 83 F8 ? 77 ? 0F B6 90");
		...
		pattern = re4t::pattern("B9 ? ? ? ? 66 89 4E ? 8B 15 ? ? ? ? F7 42");
		...
		pattern = re4t::pattern("0F BE 8C 12 ? ? ? ? 03 D2 66 ? 66 33 4E");
		...
		*/
		...
		pattern = re4t::pattern("68 ? ? ? ? E8 ? ? ? ? 6A 00 68 ? ? ? ? EB 54");
		...
		pattern = re4t::pattern("8B 0D ? ? ? ? F7 41 ? ? ? ? ? 74 ? 6A");
		...
		* 
		...
		pattern = re4t::pattern("C7 05 ? ? ? ? ? ? ? ? 0F B7 81 ? ? ? ? 8D");
	...
	// Unlock JP-only classic camera angle during Ashley segment
	{
		...
		auto pattern = re4t::pattern("8B 0D ? ? ? ? 80 79 08 00 75 ? 8A 80 A3 4F 00 00");
	...
	if (re4t::cfg->bEnableNTSCMode)
	{
		...
		auto pattern = re4t::pattern("05 7C 15 00 00 6A 00 89 81 94 4F 00 00");
		...
		pattern = re4t::pattern("8B ? ? ? ? ? 80 7E 08 01 74");
		...
		pattern = re4t::pattern("8B F9 8A ? ? 8B ? ? FE C9");
		...
		pattern = re4t::pattern("8B 15 ? ? ? ? 80 7A ? 01 74");
		...
		pattern = re4t::pattern("A1 ? ? ? ? 80 78 ? 01 75");
		...
		pattern = re4t::pattern("89 51 68 8B 57 68 8B");
		...
		pattern = re4t::pattern("C7 46 30 01 00 00 00");
	...
	// TODO: some reason the below doesn't actually let Ashley use SetKick, might be player-type checks inside it, so right now this just disables Suplex if bAllowAshleySuplex isn't set
	{
		...
		auto pattern = re4t::pattern("0F 8E ? ? ? ? A1 ? ? ? ? 80 B8 C8 4F 00 00 01 0F 84");
		injector::MakeNOP(pattern.count(1).get(0).get<uint8_t>(0x12), 6, true);

		pattern = re4t::pattern("0F B6 81 C8 4F 00 00 83 C0 FE 83 F8 03 77");
	...
	// (credits to qingsheng8848 for finding out this method!)
	{
		...
		auto pattern = re4t::pattern("89 45 FC 53 56 8B 75 08 0F B6 86 FF 00 00 00 8B 0C 85");
		...
		pattern = re4t::pattern("C6 40 24 01 0F B6 86 FF 00 00 00 8B 0C 85 ? ? ? ? 56 FF D1 83 C4 04 83 3D");
		...
		pattern = re4t::pattern("E8 ? ? ? ? 8B 96 D8 07 00 00 D9 5A 30");
	...
	{
		auto pattern = re4t::pattern("83 C4 0C E8 ? ? ? ? D9 EE 8B 06 D9 9E 44 05 00 00");
	...
	{
		auto pattern = re4t::pattern("8B 86 D8 07 00 00 8B 48 34 8B 11 8B 42 4C FF D0");
		...
		pattern = re4t::pattern("8B 8E D8 07 00 00 8B 49 34 8B 11 8B 42 4C FF");
	...
	{
		auto pattern = re4t::pattern("E8 ? ? ? ? 85 C0 74 ? 8B 96 D8 07 00 00 8B 4A 34 E8");
		...
		pattern = re4t::pattern("55 8B EC 56 8B ? ? 0F B6 86 FE 00 00 00 8B ? ? ? ? ? ? 56 FF ? 8B 8E D8 07 00 00 83 C4 04 E8 ? ? ? ? 83");
